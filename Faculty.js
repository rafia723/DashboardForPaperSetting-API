...
const getQueryWithEnabledStatus = "SELECT * FROM Faculty where status='enabled'"; //HOD
...
const searchFacultyQuery = "SELECT * FROM Faculty WHERE f_name LIKE @searchQuery OR username LIKE @searchQuery"; //Datacell
const searchFacultyQueryWithEnabledStatus = "SELECT * FROM Faculty WHERE (f_name LIKE @searchQuery OR username LIKE @searchQuery) and status='enabled'";//HOD
...
const loginfaculty="select * from faculty where username=@username and password=@password";
...
facultyRouter.post("/loginFaculty", async (req, res) => {
  try {
    ...
    // Retrieve the hashed password and fid from the database based on the provided username
    await pool.connect();
    ...
    const { f_id, password: hashedPassword } = result.recordset[0]; // Destructure f_id and hashedPassword from the result
    ...
    // If password matches, return success message along with fid
...
// SEARCH endpoint
facultyRouter.get("/searchFacultyWithEnabledStatus", async (req, res) => {
  try {
    const searchQuery = req.query.search;
    if (!searchQuery) {
      return res.status(400).json({ error: "Missing search query parameter" });
    ...
    const searchResult = await pool
      .request()
      .input("searchQuery", sql.NVarChar(255), `%${searchQuery}%`)
      .query(searchFacultyQueryWithEnabledStatus);
    res.json(searchResult.recordset);
  } catch (error) {
    console.error("Error searching faculty:", error);
    res.status(500).json({ error: "Search Request Error" });
